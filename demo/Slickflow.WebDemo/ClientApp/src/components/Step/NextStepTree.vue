...
export default {
  ...
  created () {
    ...
    this.getNextSteps(function (steps) {
      self.steps = steps
      self.$nextTick(() => {
        self.$refs.nextsteptree.expandAll()
      })
    })
  },
  data () {
  ...
  methods: {
    ...
    getNextSteps (callback) {
      ...
      axios
        .post(`${process.env.VUE_APP_BASE_URL}/wf/GetNextStepInfo`, query)
        .then((response) => {
    ...
    runProcess () {
      ...
      axios
        .post(`${process.env.VUE_APP_BASE_URL}/wf/RunProcess`, runner)
        .then((response) => {
          var result = response.data
          if (result.Status === 1) {
